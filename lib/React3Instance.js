...
var React3DInstance = (_temp = _class = function () {
    ...
    value: function _createRenderer() {
      ...
      if (window.altspace && window.altspace.inClient) {
        var _ret = function () {
          ...
          var renderer = window.altspace.getThreeJSRenderer();
          renderer.setViewport = noop;
          renderer.setSize = noop;

          _this2._renderer = renderer;
          var camera = new _three2.default.PerspectiveCamera();
          _this2._mainCamera = camera;
    ...
    value: function refreshRenderer() {
      ...
      if (contextLossExtension && this._canvas) {
        this._canvas.addEventListener('webglcontextlost', function () {
          ...
          _this3._recreateCanvasCallback();
        }, false);
  ...
  this._callOnAnimate = function () {
    if (_this4._onAnimate) {
      _ReactUpdates2.default.batchedUpdates(_this4._onAnimate);
    }
  };

  this._render = function () {
    for (var i = 0; i < _this4._modules.length; ++i) {
      _this4._modules[i].update();
    }

    _this4._renderRequest = requestAnimationFrame(_this4._render);
    _this4.userData.events.emit('animate');

    // the scene can be destroyed within the 'animate' event
    if (!_this4._scene || !_this4._mounted || !_this4._renderer) {
      return;
    }

    var mainCamera = null;

    if (_this4._mainCamera) {
      mainCamera = _this4._mainCamera;
    } else if (_this4._mainCameraName) {
      var objectsWithMainCameraName = _this4._objectsByName[_this4._mainCameraName];

      if (objectsWithMainCameraName) {
        if (process.env.NODE_ENV !== 'production') {
          (0, _warning2.default)(objectsWithMainCameraName.count < 2, 'There are multiple objects with name ' + _this4._mainCameraName);
        }
    ...
    if (mainCamera) {
      if (_this4._lastRenderMode !== 'camera') {
        _this4._renderer.autoClear = true;
        _this4._renderer.setViewport(0, 0, _this4._parameters.width, _this4._parameters.height);
        _this4._lastRenderMode = 'camera';
      }
      _CameraUtils2.default.current = mainCamera;
      _this4.userData.events.emit('preRender');
      _this4._renderScene(mainCamera);
      _CameraUtils2.default.current = null;
    } else if (_this4._viewports.length > 0) {
      if (_this4._lastRenderMode !== 'viewport') {
        _this4._renderer.autoClear = false;
        _this4._lastRenderMode = 'viewport';
      }

      _this4._renderer.clear();
      _this4._viewports.forEach(function (viewport) {
        var viewportCamera = null;

        if (viewport.cameraName) {
          var objectsWithViewportCameraName = _this4._objectsByName[viewport.cameraName];
        ...
        _this4._renderer.setViewport(viewport.x, viewport.y, viewport.width, viewport.height);
        _CameraUtils2.default.current = viewportCamera;
        _this4.userData.events.emit('preRender');
        _this4._renderScene(viewportCamera);
        _CameraUtils2.default.current = null;
      });
    }
  };
